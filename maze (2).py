...
class Maze:
    ...
    def __init__(self, mazeLayer, solveLayer):
        ...
        for y in xrange(ScreenHeigth): # 80 wide + 60 tall
            pygame.draw.line(self.mLayer, (0,0,0,255), (0, y*8), (640, y*8))
            for x in xrange(ScreenWidth):
                self.mazeArray.append(0)
                if ( y == 0 ):
                    pygame.draw.line(self.mLayer, (0,0,0,255), (x*8,0), (x*8,8*ScreenHeigth))
        pygame.draw.rect(self.sLayer, (0,0,255,255), Rect(0,0,8,8))
        pygame.draw.rect(self.sLayer, (255,0,255,255), Rect(((8*ScreenWidth)-8),((8*ScreenHeigth)-9),8,8))
        # Maze Section
        self.totalCells = 4800 # 80 * ScreenHeigth
    ...
    def update(self):
        if self.state == 'c':
            ...
            while(self.visitedCells < self.totalCells):# and moved == False):#build here
                x = self.currentCell % ScreenWidth
                y = self.currentCell / ScreenWidth
                neighbors = []
                for i in xrange(4):
                    nx = x + self.compass[i][0]
                    ny = y + self.compass[i][1]
                    if ((nx >= 0) and (ny >= 0) and (nx < ScreenWidth) and (ny < ScreenHeigth)):
                        if (self.mazeArray[(ny*ScreenWidth+nx)] & 0x000F) == 0:
            ...
            with open("maze/maze.csv", "a") as myfile:
                myfile.write(str(self.iteration-1) + " , "+ str(len(self.cellStack)) + " , " +
                    ...
                    str((ScreenWidth*ScreenHeigth)-len(self.badStack)-len(self.cellStack)) + " , " +
                    str("{0:.2f}".format(100*float((len(self.cellStack))/float((ScreenWidth*ScreenHeigth))))) + " , " +
                    str("{0:.2f}".format(100*float(len(self.badStack))/float((ScreenWidth*ScreenHeigth)))) + " , " + "\n")
...
def main():
    ...
    pygame.init()
    myScreen = pygame.display.set_mode((640, 8*ScreenHeigth))
